...
interface StructuredLinearData {
  projects: Project[];
  tasks: { [project: string]: { [status: string]: Task[] } };
}
...
async function fetchMyProjects(): Promise<Project[]> {
  ...
  const myProjects: Project[] = await Promise.all(
  ...
  myProjects.forEach((project) => {
    if (project.targetDate) {
      const remainingWorkDays = calculateWorkDaysRemaining(
        currentDate,
 ...
 */
async function fetchCompletedIssues(): Promise<Task[]> {
  const me = await linearClient.viewer;
 ...
 */
async function fetchUpcomingIssues(): Promise<Task[]> {
  const me = await linearClient.viewer;
...
function sortProjectsAlphabetically(data: {
  [project: string]: { [status: string]: Task[] };
}): { [project: string]: { [status: string]: Task[] } } {
  const sortedData: { [project: string]: { [status: string]: Task[] } } = {};
 ...
 */
async function fetchDataFromLinear(): Promise<StructuredLinearData> {
  try {
    ...
    const structuredData: { [project: string]: { [status: string]: Task[] } } =
      {};
 ...
 */
function calculateWorkDaysRemaining(startDate: Date, targetDate: Date): number {
  ...
  const diffDays = Math.round(
    Math.abs((start.getTime() - target.getTime()) / oneDay)
  );
 ...
 */
function generateMarkdownReport(data: StructuredLinearData): string {
  let report = "";
 ...
 */
function generateHTMLReport(data: StructuredLinearData): string {
